package ChangelogMai2026 is
   pragma Pure;
   
   -- Version 0.06. => 0.06. (31.05.2026):
   
   -- 
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06. => 0.06. (30.05.2026):
   
   -- 
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06. => 0.06. (29.05.2026):
   
   -- 
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06. => 0.06. (28.05.2026):
   
   -- 
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06. => 0.06. (27.05.2026):
   
   -- 
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06. => 0.06. (26.05.2026):
   
   -- 
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06. => 0.06. (25.05.2026):
   
   -- 
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06. => 0.06. (24.05.2026):
   
   -- 
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06. => 0.06. (23.05.2026):
   
   -- 
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06. => 0.06. (22.05.2026):
   
   -- 
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06. => 0.06. (21.05.2026):
   
   -- 
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06. => 0.06. (20.05.2026):
   
   -- 
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06.4080 => 0.06. (19.05.2026):
   
   -- 
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06.4055 => 0.06.4080 (18.05.2026):
   
   -- Changelog Vorlagen für 2027 eingefügt.
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06.4020 => 0.06.4055 (17.05.2026):
   
   -- Spieleentwicklungsbibliothek von Version 0.01.2200 auf Version 0.01.2400 aktualisiert.
   -- Lokale Pragmas angepasst, damit es keine Probleme mit den Änderungen in der Spieleentwicklungsbibliothek gibt.
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06.3995 => 0.06.4020 (16.05.2026):
   
   -- Unifont von Version 17.0.04 auf Version 17.0.05 aktualisiert.
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06.3970 => 0.06.3995 (15.05.2026):
   
   -- Spieleentwicklungsbibliothek von Version 0.01.2150 auf Version 0.01.2200 aktualisiert.
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06.3945 => 0.06.3970 (14.05.2026):
   
   -- GNAT 14.3.0-16 auf GNAT 14.4.0-1 aktualisiert.
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06.3920 => 0.06.3945 (13.05.2026):
   
   -- Lokale Pragmas überarbeitet und erweitert.
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06.3890 => 0.06.3920 (12.05.2026):
   
   -- Speichersystem weiter überarbeitet.
   -- Kommentare und Kleinigkeiten korrigiert/angepasst/entfernt.
   
   

   -- Version 0.06.3865 => 0.06.3890 (11.05.2026):
   
   -- GNAT 14.3.0-15 auf GNAT 14.3.0-16 aktualisiert.
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06.3840 => 0.06.3865 (10.05.2026):
   
   -- GNAT 14.3.0-14 auf GNAT 14.3.0-15 aktualisiert.
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06.3795 => 0.06.3840 (09.05.2026):
   
   -- Weiter am Speichersystem gearbeitet.
   -- Interne Benennung überarbeitet.
   -- Einige Datentypen durch die Entwicklungsbibliotheksversionen ersetzt.
   -- Contracts und Kleinigkeiten korrigiert/angepasst.
   
   

   -- Version 0.06.3770 => 0.06.3795 (08.05.2026):
   
   -- Spieleentwicklungsbibliothek von Version 0.01.2100 auf Version 0.01.2150 aktualisiert.
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06.3740 => 0.06.3770 (07.05.2026):
   
   -- Weiter am Speichersystem gearbeitet.
   -- Kommentare, Contracts und Kleinigkeiten korrigiert/angepasst/überarbeitet.
   
   

   -- Version 0.06.3715 => 0.06.3740 (06.05.2026):
   
   -- Benennung von Unifont angepasst um weniger Änderungen vornehmen zu müssen wenn eine neue Version veröffentlicht wird.
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06.3690 => 0.06.3715 (05.05.2026):
   
   -- Spieleentwicklungsbibliothek von Version 0.01.2050 auf Version 0.01.2100 aktualisiert.
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06.3650 => 0.06.3690 (04.05.2026):
   
   -- Speichersystem überarbeitet.
   -- Einige Datentypen durch die Entwicklungsbibliotheksversionen ersetzt.
   -- Kommentare und Kleinigkeiten korrigiert/angepasst/überarbeitet.
   
   

   -- Version 0.06.3625 => 0.06.3650 (03.05.2026):
   
   -- Fehler in der Pfadangabe für Unifont korrigiert.
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06.3600 => 0.06.3625 (02.05.2026):
   
   -- Interne Benennung überarbeitet.
   -- Kleinigkeiten korrigiert.
   
   

   -- Version 0.06.3575 => 0.06.3600 (01.05.2026):
   
   -- Interne Benennung überarbeitet.
   -- Kleinigkeiten korrigiert.

end ChangelogMai2026;
