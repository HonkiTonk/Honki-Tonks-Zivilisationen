...
TextaccessverwaltungssystemGrafik.TextPositionZeichnen (TextaccessExtern => TextaccessVariablen.TextAccess,
                                                        ...
                                                        PositionExtern   => (PositionExtern.x, (FeldgrößeExtern.y / 5.00)
...
package body StadtumgebungGrafik is
   ...
   begin
      ...
      -- Die farben hier später auch vom Nutzer belegbar machen. äöü
      if
         ...
         is
            when True =>
               ...
               Wirtschaftsinformationen (KoordinatenExtern => KarteKoordinatenExtern,
                                         ...
                                         FeldgrößeExtern   => (FeldgrößeExtern.x - 2.00 * Rahmendicke, FeldgrößeExtern.y - 2.00 * Rahmendicke),
                                         SpeziesExtern     => StadtSpeziesNummerExtern.Spezies);

               -- Später den View mit rein geben. äöü
               ViewsEinstellenGrafik.ViewSetzen (ViewExtern => Views.StadtviewAccesse (ViewKonstanten.StadtUmgebung));
   ...
   is begin
      ...
      ViewflächeFeldproduktion := ViewsEinstellenGrafik.ViewflächeVariabelAnpassen (ViewflächeExtern => ViewflächeFeldproduktion,
                                                                                      VerhältnisExtern => (1.00, 1.00));

      ViewsEinstellenGrafik.ViewEinstellen (ViewExtern           => Views.InformationsfeldAccesse (ViewKonstanten.InformationenFeldproduktion),
                                            GrößeExtern          => ViewflächeFeldproduktion,
      ...
      for ProduktionSchleifenwert in ProduktionDatentypen.Produktion_Enum'Range loop
         ...
         TextaccessverwaltungssystemGrafik.TextPositionZeichnen (TextaccessExtern => TextaccessVariablen.TextAccess,
                                                                 TextExtern       => To_Wide_Wide_String (Source => Text),
                                                                 PositionExtern   => (TextberechnungenBreiteGrafik.KleinerSpaltenabstandVariabel,
                                                                                      Float (ProduktionDatentypen.Produktion_Enum'Pos (ProduktionSchleifenwert))
                                                                                      * TextberechnungenHoeheGrafik.KleinerZeilenabstandVariabel));

      end loop ProduktionSchleife;

      GrafikVariablen.InformationsfeldBereiche (ViewKonstanten.InformationenFeldproduktion)
        := ViewbereicheBerechnenGrafik.Test (BereichExtern => GrafikVariablen.InformationsfeldBereiche (ViewKonstanten.InformationenFeldproduktion));

      ViewsEinstellenGrafik.ViewEinstellen (ViewExtern           => Views.InformationsfeldAccesse (ViewKonstanten.InformationenFeldproduktion),
                                            GrößeExtern          => FeldgrößeExtern,
                                            AnzeigebereichExtern => GrafikVariablen.InformationsfeldBereiche (ViewKonstanten.InformationenFeldproduktion));
