...
package body ZeigerplatzierungGrafik is
   ...
   begin
      ...
      if
        ...
        or else
          (Float'Floor (Mausposition.y / SichtweitenGrafik.Kartenfeldfläche.y) > Float (KartenDatentypen.SenkrechteBasis'Last)
           or
             Float'Floor (Mausposition.y / SichtweitenGrafik.Kartenfeldfläche.y) < Float (KartenDatentypen.SenkrechteBasis'First))
      then
         Kartenänderung.Senkrechte := -Sichtbereich.Senkrechte;
         
      else
         Kartenänderung.Senkrechte := -Sichtbereich.Senkrechte + KartenDatentypen.SenkrechteBasis (Float'Floor (Mausposition.y / SichtweitenGrafik.Kartenfeldfläche.y));
      end if;
      
      if
        SichtweitenGrafik.Kartenfeldfläche.x = GrafikKonstanten.Nullwert
        or else
          (Float'Floor (Mausposition.x / SichtweitenGrafik.Kartenfeldfläche.x) > Float (KartenDatentypen.SenkrechteBasis'Last)
           or
             Float'Floor (Mausposition.x / SichtweitenGrafik.Kartenfeldfläche.x) < Float (KartenDatentypen.SenkrechteBasis'First))
      then
